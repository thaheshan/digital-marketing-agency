...
export const getPublishedItems = async () => {
  return prisma.portfolioItem.findMany({ where: { status: "published" }, include: { images: true, metrics: true, service: true }, orderBy: { sortOrder: "asc" } });
};

export const getItemBySlug = async (slug: string) => {
  const item = await prisma.portfolioItem.findUnique({ where: { slug }, include: { images: true, metrics: true, service: true } });
...
export const createItem = async (data: any, userId: string) => {
  ...
  return prisma.portfolioItem.create({ data: { slug, title: data.title, clientName: data.clientName, clientIndustry: data.clientIndustry, serviceCategory: data.serviceCategory, challengeText: data.challengeText, solutionText: data.solutionText, status: "draft", createdBy: userId, metrics: { create: data.metrics || [] } } });
};

export const updateItem = async (id: string, data: any) => {
  return prisma.portfolioItem.update({ where: { id }, data: { title: data.title, clientName: data.clientName, clientIndustry: data.clientIndustry, serviceCategory: data.serviceCategory, description: data.description, challengeText: data.challengeText, solutionText: data.solutionText, status: data.status, selectedAiOption: data.selectedAiOption } });
};

export const deleteItem = async (id: string) => prisma.portfolioItem.delete({ where: { id } });
...
export const generateDescription = async (id: string, context: any) => {
  try {
    ...
    await prisma.portfolioItem.update({ where: { id }, data: { aiGeneratedOption1: opt1, aiGeneratedOption2: opt2, aiGeneratedOption3: opt3 } });
    return { option_1: opt1, option_2: opt2, option_3: opt3 };
  } catch (err) {
    console.error("[ML Proxy] Portfolio generator failed:", err);
    return { option_1: "AI engine warming up.", option_2: "AI engine warming up.", option_3: "AI engine warming up." };
  }
};

...
export const trackVisitor = async (data: any, ipAddress: string, userAgent: string) => {
  ...
  if (!visitor) {
    ...
    await prisma.siteVisitor.update({
      where: { id: visitor.id },
      data: { lastSeenAt: new Date(), totalPageViews: { increment: eventType === "page_view" ? 1 : 0 } }
    });
  }
  if (eventType) {
    await prisma.visitorEvent.create({
      data: {
        visitorId: visitor.id, eventType, pageUrl, pageTitle, timeOnPageSeconds,
  ...
  try {
    ...
    if (response.ok) {
      ...
      await prisma.enquiry.update({
        where: { id: enquiry.id },
        data: { leadScore: score, leadTemperature: temperature as "hot" | "warm" | "cold" }
      });
      await prisma.enquiryLeadScore.create({
        data: {
          enquiryId:     enquiry.id,
          behaviourScore:mlResult.behaviour_score || 0,
          formScore:     mlResult.form_score || score,
          totalScore:    score,

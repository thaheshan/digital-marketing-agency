...
export const processMessage = async (data: any) => {
  ...
  try {
    ...
    if (session) {
      if (message) {
        await prisma.chatbotMessage.create({ data: { sessionId: session.id, sender: "user", messageText: message } });
      }
...
export const updateKnowledgeBaseItem = async (id: string, data: any) => {
  return prisma.chatbotKnowledgeBase.update({
    where: { id }, data: { question: data.question, answer: data.answer, isActive: data.isActive }
  });
};
